...
class PaymentService {
  ...
  constructor() {
    ...
    this.backendBaseUrl = import.meta.env.VITE_API_BASE_URL || 'https://qwiky-backend.onrender.com/api';
  ...
  async initiatePayment(paymentData: PaymentRequest): Promise<InitiatePaymentResponse> {
    try {
      ...
      const orderResponse: CashfreeOrderResponse = response.data;
  ...
  async verifyPayment(orderId: string): Promise<boolean> {
    try {
      ...
      console.log('Cashfree payment verification result:', response.data);
      
      const isSuccess = response.data.payment_status === 'SUCCESS' || 
  ...
  async testPaymentService(): Promise<{ success: boolean; mode: string; message: string }> {
    try {
      ...
      try {
        const response = await apiClient.get('/payment/test');
        console.log('Backend API test successful:', response.data);
        
        return {
          success: true,
          mode: this.paymentMode,
          message: `Cashfree ${this.isTestMode() ? 'Sandbox' : 'Production'} mode ready. Backend API connected.`
        };
